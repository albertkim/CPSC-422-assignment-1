...
class Grid {
  ...
  private calculateNewCurrentStateForCoordinate(coordinate: Coordinate, direction: Direction, observation: 1| 2): void {
    ...
    if (direction === Direction.UP) {
      ...
      if (!canComeFromBottom) { selfTransitionProbability += 0.8 }
      if (!canComeFromLeft) { selfTransitionProbability += 0.1 }
      if (!canComeFromRight) { selfTransitionProbability += 0.1 }
      adjacentStates.push({
        coordinate: coordinate,
        transitionProbability: selfTransitionProbability,
        currentProbability: this.getCurrentProbabilityAtCoordinate(coordinate) as number
      })
